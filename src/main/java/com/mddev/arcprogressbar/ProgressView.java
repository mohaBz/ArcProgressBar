...
package com.mddev.arcprogressbar;
...
import com.mddev.arcprogressbar.utils.OnProgressViewListener;
import com.mddev.arcprogressbar.utils.ProgressShape;
import com.mddev.arcprogressbar.utils.ProgressStartPoint;
...
abstract class ProgressView extends View implements ProgressShape {
    ...
    protected int width;
    protected int min;
    protected float dashLenght=getResources().getDimension(R.dimen.stroke_dashlenght);
    protected  float dashGap=dashLenght=getResources().getDimension(R.dimen.stroke_dashgap);;
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.ArcProgressBar, 0, 0);
        try {
            progress = typedArray.getFloat(
                    R.styleable.ArcProgressBar_progress, progress);
            strokeWidth = typedArray.getDimension(
                    R.styleable.ArcProgressBar_progress_width, strokeWidth);
            backgroundStrokeWidth = typedArray.getDimension(
                    R.styleable.ArcProgressBar_bar_width,
                    backgroundStrokeWidth);
            color = typedArray.getInt(
                    R.styleable.ArcProgressBar_progress_color, color);
            backgroundColor = typedArray.getInt(
                    R.styleable.ArcProgressBar_bar_color, backgroundColor);
            text_data.textColor = typedArray.getInt(
                    R.styleable.ArcProgressBar_text_color,
                    text_data.textColor);
            text_data.textSize = typedArray
                    .getInt(R.styleable.ArcProgressBar_text_size,
                            text_data.textSize);
            dashMode = typedArray
                    .getBoolean(R.styleable.ArcProgressBar_dash_stroke,
                            dashMode);
            dashLenght=typedArray
                    .getDimension(R.styleable.ArcProgressBar_dash_lenght
                            ,dashLenght);
            dashGap=typedArray
                    .getDimension(R.styleable.ArcProgressBar_dash_gap,
                            dashGap);
        } finally {
            typedArray.recycle();
        }
    ...
    protected void initForegroundColor() {
        ...
        foregroundPaint.setStrokeWidth(strokeWidth);
        if(dashMode){
            foregroundPaint.setPathEffect(new DashPathEffect(new float[]{dashLenght,dashGap},0));
        }
        if (isRoundEdge) {
    ...
    protected void initBackgroundColor() {
        ...
        if(dashMode){
            backgroundPaint.setPathEffect(new DashPathEffect(new float[]{dashLenght,dashGap},0));
        }
